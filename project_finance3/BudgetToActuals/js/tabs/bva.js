...
async function loadBudgetData(months) {
  const [labor, direct, employees] = await Promise.all([
    client.from('budget_labor').select('*').eq('grant_id', currentGrant.id),
    client.from('budget_direct').select('*').eq('grant_id', currentGrant.id),
    client.from('labor_categories').select('id, name').eq('is_active', true)
  ]);
  ...
  labor.data?.forEach(r => {
    const name = empMap.get(r.employee_id) || 'Unknown';
    months.forEach(m => {
      const hours = r[`hours_${m}`] || 0;
  ...
  direct.data?.forEach(r => {
    months.forEach(m => {
      const amount = r[`amount_${m}`] || 0;
      const key = `${r.category}-${m}`;
      directMap.set(key, (directMap.get(key) || 0) + amount);
    });
  });
...
function renderTable(months, budget, actuals) {
  ...
  cats.forEach(cat => {
    ...
    const cells = months.map(ym => {
      const b = (budget.laborMap.get(`${cat}-${ym}`) || 0) + (budget.directMap.get(`${cat}-${ym}`) || 0);
      const a = actuals.get(`${cat}-${ym}`) || 0;
      rowB += b; rowA += a;
      const v = b - a;
      const cls = v > 0 ? 'text-success font-medium' : v < 0 ? 'text-danger font-bold' : '';
      return `<td class="px-4 py-3 text-right ${cls}">${fmt(v)}</td>`;
    }).join('');
    const totalV = rowB - rowA;
    const totalCls = totalV > 0 ? 'text-success font-medium' : 'text-danger font-bold';
    html += `<tr class="hover:bg-slate-50">
      <td class="px-4 py-3 sticky left-0 bg-white font-medium text-slate-700">${esc(cat)}</td>
      ${cells}
      <td class="px-4 py-3 text-right font-medium ${totalCls}">${fmt(totalV)}</td>
    </tr>`;
    grandB += rowB; grandA += rowA;
  });

  const totalCells = months.map(ym => {
    let b = 0, a = 0;
    cats.forEach(c => {
      b += (budget.laborMap.get(`${c}-${ym}`) || 0) + (budget.directMap.get(`${c}-${ym}`) || 0);
  ...
  const grandV = grandB - grandA;
  const grandCls = grandV > 0 ? 'text-success font-medium' : 'text-danger font-bold';
  html += `<tr class="bg-slate-100 font-bold">
    <td class="px-4 py-3 sticky left-0 bg-slate-100">TOTAL</td>
    ${totalCells}
    <td class="px-4 py-3 text-right ${grandCls}">${fmt(grandV)}</td>
  </tr>`;
...
function renderCharts(months, budget, actuals) {
  ...
  const budgetData = months.map(ym => {
    let tot = 0;
    budget.laborMap.forEach((v, k) => { if (k.endsWith(`-${ym}`)) tot += v; });
    budget.directMap.forEach((v, k) => { if (k.endsWith(`-${ym}`)) tot += v; });
    return tot;
  });
  const actualData = months.map(ym => {
    let tot = 0;
    actuals.forEach((v, k) => { if (k.endsWith(`-${ym}`)) tot += v; });
    return tot;
  });
...
function updateSummary(budget, actuals) {
  let totalB = 0;
  budget.laborMap.forEach(v => totalB += v);
  budget.directMap.forEach(v => totalB += v);
  let totalA = 0;
  actuals.forEach(v => totalA += v);
