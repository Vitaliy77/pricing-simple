...
// ─────────────────────────────────────────────
// DYNAMIC STICKY COLUMN FIX (eliminates gaps)
// ─────────────────────────────────────────────
function fixStickyOffsets(root) {
  ...
  // Measure first 3 columns and apply exact pixel-perfect left offsets
  [0, 1, 2].forEach((idx) => {
    const colWidth = headerCells[idx].offsetWidth;
...
export const revenueBudgetTab = {
  ...
  async init({ root, client }) {
    ...
    const globalPlan = document.querySelector("#planContextHeader")?.textContent?.trim() || "";
    const globalProject = document.querySelector("#currentProject")?.textContent?.trim() || "";

    $("#revInlinePlan", root).textContent = globalPlan;
    $("#revInlineProject", root).textContent = globalProject ? `, ${globalProject}` : "";
...
// ─────────────────────────────────────────────
// RENDER REVENUE (with sticky fix applied)
// ─────────────────────────────────────────────
function renderRevenue(root, rows) {
  ...
  // CRITICAL: Fix sticky column gaps after table is fully rendered
  fixStickyOffsets(root);
}

// (All other functions remain unchanged: loadProjectsUnderLevel1, loaders, refreshRevenue, insert, update, etc.)
// ... [rest of your file — unchanged from your original]
...
// ... rest of your existing functions (loadTmRevenueRows, loadSubsOdcRevenueRows, etc.) remain 100% unchanged
// (They are already perfect — no need to repeat here)
