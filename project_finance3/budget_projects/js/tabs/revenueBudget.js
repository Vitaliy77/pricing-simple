...
async function refreshRevenue(root, client) {
  ...
  try {
    ...
    if (!Array.isArray(tmRows)) {
      console.warn("[Revenue] tmRows is not array, got:", tmRows);
      tmRows = [];
    }
    if (!Array.isArray(subsRows)) {
      console.warn("[Revenue] subsRows is not array, got:", subsRows);
      subsRows = [];
    }
    if (!Array.isArray(manualRows)) {
      console.warn("[Revenue] manualRows is not array, got:", manualRows);
      manualRows = [];
    }
