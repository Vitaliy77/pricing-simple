...
export const summaryPlanTab = {
  ...
  async init({ root, client }) {
    // Clear any existing charts when re-initializing
    destroyCharts();
...
// ─────────────────────────────────────────────
// PROJECT SCOPE (same idea as P&L)
// ─────────────────────────────────────────────
...
// B) Manual revenue types from planning_lines
...
// C) Cost by type from planning_lines
...
// D) Subs & ODC revenue = subc + odc cost
async function loadSubsOdcRevenueFromCost(client, ctx) {
  ...
  return { subsOdcRev, costByType }; // return both so we don't reload costs
}
...
async function computeSummary(client, ctx) {
  ...
  // Combine labor cost from hours + DIR_LAB_COST planning_lines
...
// ─────────────────────────────────────────────
// RENDER – CHARTS (Chart.js if available)
// ─────────────────────────────────────────────
...
function renderCharts(root, summary) {
  ...
  // 1) Stacked bar – Monthly revenue by type
  if (revByMonthCanvas) {
    ...
    const chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            label: "T&M Labor",
            data: monthlyRevenueByType.tm,
            backgroundColor: revColors.tm,
          },
          {
            label: "Fixed",
            data: monthlyRevenueByType.fixed,
            backgroundColor: revColors.fixed,
          },
          {
            label: "Software",
            data: monthlyRevenueByType.software,
            backgroundColor: revColors.software,
          },
          {
            label: "Unit",
            data: monthlyRevenueByType.unit,
            backgroundColor: revColors.unit,
          },
          {
            label: "Other",
            data: monthlyRevenueByType.other,
            backgroundColor: revColors.other,
          },
          {
            label: "Subs & ODC",
            data: monthlyRevenueByType.subsOdc,
            backgroundColor: revColors.subsOdc,
          },
        ],
      },
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label(context) {
                const value = context.parsed.y || 0;
                return `Profit: ${fmtCurrency(value)}`;
              },
            },
          },
        },
          ...
          tooltip: {
            callbacks: {
              label(context) {
                ...
                return `${label}: ${fmtCurrency(value)} (${fmtPercent(pct)})`;
              },
            },
          },
        },
